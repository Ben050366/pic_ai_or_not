...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (contentType.includes('multipart/form-data')) {
            ...
            const formData = await parseMultipart(bodyBuffer, contentType);
            ...
            const blob = new Blob([formData.file.content], { type: formData.file.type || 'image/jpeg' });
            hiveForm.append('media', blob, formData.file.name || 'image.jpg');
            
            const hiveResponse = await fetch(HIVE_API_URL, {
                method: 'POST',
                headers: {
        ...
        if (nameMatch) {
            ...
            if (filenameMatch) {
                ...
                const fileBuffer = buffer.slice(
                    bufferStr.indexOf(part, 0, 'binary') + contentStart,
                    bufferStr.indexOf(part, 0, 'binary') + contentEnd
                );
